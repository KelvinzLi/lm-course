...
from ttlm.dataset.tinyshakespeare import TinyShakespeare
...
def pretrain(config: PreTrainingConfig) -> None:
    ...
    with World(device=config.device) as world:
        ...
        dataset = TinyShakespeare(parquet_path=config.data.parquet_path)
        sampler = (
        ...
        if world.distributed: model = DDP(model, device_ids=[world.local_rank])
        ...
        for epoch in range(config.epochs):
            ...
            for i, batch in enumerate(dataloader):
                ...
                tensor_ids = pad_sequence(input_ids, batch_first=True, padding_value=tokenizer.pad_token_id).to(world.device)
                ...
                loss = torch.nn.functional.cross_entropy(pred_logits, labels, ignore_index=tokenizer.pad_token_id)
                loss.backward()
